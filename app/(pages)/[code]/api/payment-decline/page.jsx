...
export default async function PaymentCancelPage({ searchParams, params }) {
  ...
  if (up && typeof up === "string") {
    try {
      ...
      // 4. Decline Webhook-a POST sorğusu göndəririk
      await fetch(
        "https://payments.tezz.az/api/united-payment/webhook/decline/",
        {
          method: "POST",
          headers: {
      ...
      console.log("Decline webhook sent successfully");
    } catch (err) {
      console.error("Payment decline webhook error:", err);
    }
  }
  return (
    <>
